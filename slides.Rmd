...
The core of `pagedown` customization power relies on the `layout.css` file, which handle :
...
+ you can (and must) set it's size at the beginning 
...
Some special pages exists (they are named pseudo-classes) like :
...
This system is useful to style every margin box separately :
...
Margin size is controlled by CSS variable in `vars.css`. They are automatically handled by Pagedjs after.
...
**Tip** : you should set the dimension of your page and margins first before to setup everything else.

--

**Tip** : if you want an image (like a logo) in a margin box, put it as a background image and set `content:"";`
...
Table of content can be set and managed through an instruction given by the default CSS of `pagedown` :
